...
from common import readLine, revComp
...
def generateKeys(dnaString, k, d):
    ...
    for s in subs:
        revMap[s] = revComp(s)
    return (list(subs), revMap)

def run(que, dnaString, d, keys, revMap, tid, numWork):
    freq = {}
    cnt = 0
    start = numWork * tid
    end = numWork * (tid + 1)
    ...
    for i in range(start,end):
        ...
        freq[k] += countApproxMatch(dnaString, revMap[k], d)
        if cnt % 5000 == 0:
...
def mostFreq(dnaString, k, d):
    (subs, revMap) = generateKeys(dnaString, k, d)
    numSubs = len(subs)
    numThreads = 8
    ...
    for tid in range(0, numThreads):
        q = Queue()
        p = Process(target=run, args=(q, dnaString, d, subs, revMap, tid, numWork))
        threads.append(p)
        queues.append(q)
    for t in threads:
        t.start()
